...
void touchscreen_process_x(unsigned char adc_value)
{
			...
			if (x_loc>15)   //The natural idle value of the touch screens is about 2 for the x and 2 for the y 
				...
				if (x_loc>x_loc_max) //here we look for the max x value durring a press (based on the max of all the sample counts)
					...
					lcd_puts(msg,0,50,c,blk);

					strncpy_P(msg,PSTR("Please powercycle"),40);
					lcd_puts(msg,0,60,c,blk);
					while (1)
					{
					;
					}
...
lcd_puts("Touch to calibrate",18,5,c1,c2);


lcd_circle(20,60,6,c1,c2); //draw
press = no;
while(!press)
	{
	;
...
lcd_circle(20,60,6,c2,c2); //erase
delay_ms(250);
...
lcd_circle(60,20,6,c2,c2); //erase
delay_ms(250);
...
lcd_circle(90,115,6,c1,c2); //draw
press = no;
while(!press)
	{
	;
...
lcd_circle(90,115,6,c2,c2); //erase

COLOR c = {200,200,200};
COLOR blk = {0,0,0};

lcd_clearScreen(blk);
lcd_puts("Calibrating the",15,15,c,blk);
lcd_puts("Touch Screen",20,25,c,blk);
